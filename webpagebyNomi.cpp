...
{
	int ResponseSize, NumberOfRequest;
	Node *head;
	fstream MyFile;
public:
	FileManagement()
	{
		head = NULL;
		NumberOfRequest = ResponseSize = 0;
	}
	void readFromFile();
...
void FileManagement::SeparateData(string str)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		else if (spaces == 6 && str[i]!='?'&& flag) // separating Name
		{
			name[index++] = str[i];
		}
		else if ((str[i] == '?' || spaces == 7) && flag) // 
		{
			flag = false;
			name[index] = '\0';
			index = 0;
		}
		else if (spaces == 9 && str[i]>='0' && str[i]<='9') // separating response size
		{
			str[index++] = str[i];
		}
		else
